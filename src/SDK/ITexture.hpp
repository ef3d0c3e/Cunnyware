#ifndef SDK_ITEXTURE_HPP
#define SDK_ITEXTURE_HPP

#include "IVTFTexture.hpp"

class ITexture;
class ITextureRegenerator : non_constructible
{
public:
	...
	virtual void RegenerateTextureBits(ITexture * texture, IVTFTexture* VTFTexture, Rect2i* rect) = 0;
...
public:
	...
	virtual void* GetResourceData(u32 eDataType, size_t * pNumBytes) const = 0;
	...
	virtual void SetTextureRegenerator(ITextureRegenerator * textureRegen, bool releaseExisting = true) = 0;
	...
	virtual void Download(Rect2i* rect = 0) = 0;

	// Uses for stats. . .get the approximate size of the texture in it's current format.
	virtual i32 GetApproximateVidMemBytes(void) const = 0;
	...
	virtual i32 GetMappingDepth() const = 0;
	virtual i32 GetActualDepth() const = 0;
	...
	virtual u32 GetFlags(void) const = 0;

	// Force LOD override (automatically downloads the texture)
	virtual void ForceLODOverride(i32 iNumLodsOverrideUpOrDown) = 0;

	// Force exclude override (automatically downloads the texture)
	virtual void ForceExcludeOverride(i32 iExcludeOverride) = 0;
};

inline bool IsErrorTexture(ITexture* tex)
{
	return !tex || tex->IsError();
}

#endif // SDK_ITEXTURE_HPP
