...
template <bool x64Mode, DemanglerType Demangler = DemanglerType::NONE>
class ELF
{
	using Ehdr = std::conditional_t<x64Mode, Elf64_Ehdr, Elf32_Ehdr>;
	using Shdr = std::conditional_t<x64Mode, Elf64_Shdr, Elf32_Shdr>;
	using Phdr = std::conditional_t<x64Mode, Elf64_Phdr, Elf32_Phdr>;
	using Half = std::conditional_t<x64Mode, Elf64_Half, Elf32_Half>;
	using Word = std::conditional_t<x64Mode, Elf64_Word, Elf32_Word>;
	using Xword = std::conditional_t<x64Mode, Elf64_Xword, Elf32_Xword>;
	using Sword = std::conditional_t<x64Mode, Elf64_Sword, Elf32_Sword>;
	using Addr = std::conditional_t<x64Mode, Elf64_Addr, Elf32_Addr>;
	using Off = std::conditional_t<x64Mode, Elf64_Off, Elf32_Off>;
	using ESection = std::conditional_t<x64Mode, Elf64_Section, Elf32_Section>;
	using Esym = std::conditional_t<x64Mode, Elf64_Sym, Elf32_Sym>;
...
public:
	...
	struct Section
	{
		...
		Addr address;
		Off offset;
		std::conditional_t<x64Mode, Xword, Word> size;
		Word link;
		Word info;
		std::conditional_t<x64Mode, Xword, Word> addrAlign;
		std::conditional_t<x64Mode, Xword, Word> entSize;
	};
	...
	struct Segment
	{
		...
		Addr vaddr;
		Addr paddr;
		std::conditional_t<x64Mode, Xword, Word> fileSize;
		std::conditional_t<x64Mode, Xword, Word> memSize;
		std::conditional_t<x64Mode, Xword, Word> align;
		SegmentFlag flags;
	};

	struct Symbol
	{
		std::conditional_t<x64Mode, Xword, Word> num;
...
public:
	...
	static constexpr void GetSections(std::uintptr_t library, std::function<void(Section&&)> callback)
	{
		...
		const char* const strTab = reinterpret_cast<const char*>(library) + shdr[ehdr->e_shstrndx].sh_offset;
	...
	static constexpr void GetSymbols(std::uintptr_t library, std::function<void(Symbol&&)> callback)
	{
		...
		for(const auto& sec: secs)
		{
			...
			for (std::conditional_t<x64Mode, Xword, Word> i = 0; i < total_syms; ++i)
			{
				Symbol s;
				s.num   = i;
...
static inline bool is64Bit(std::uintptr_t library)
{
	return reinterpret_cast<const std::uint8_t*>(library)[EI_CLASS] == 2;
}
